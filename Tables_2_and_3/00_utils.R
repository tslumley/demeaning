...
get_outcome_formulas <- function(YScenario = "1", plasmode = FALSE, rare_outcome = FALSE) {
  if (!plasmode) {
    ...
    population_formula <- gsub("_obs", "", outcome_formula)
  }
...
cover <- function(mu,est,SE,scale = "identity"){
  ...
  covers <- ifelse(!is.na(mu) & (is.na(ci_ll) | is.na(ci_ul)), FALSE, (ci_ll <= mu) * (ci_ul >= mu))
  return(covers)
...
# get nice y scenario label
# @param yscenario the character-valued y-scenario
get_nice_yscenario_label <- function(yscenario = "1.1") {
  if (yscenario == "1" | yscenario == "1.1" | yscenario == "1.15" | yscenario == "1.16" | yscenario == "1.17") {
    y_label <- "simple_outcome"
    if (yscenario == "1") {
      y_label <- paste0(y_label, "_notx")
    }
    if (yscenario %in% c("1.15", "1.16", "1.17")) {
      y_label <- paste0(y_label, "_rare")
    }
  } else if (grepl("3.", yscenario, fixed = TRUE)) {
    y_label <- "semicomplex_outcome"
    if (grepl(".1", yscenario, fixed = TRUE) | grepl(".2", yscenario, fixed = TRUE)) {
      y_label <- paste0(y_label, "_z")
    } else {
      y_label <- paste0(y_label, "_w")
    }
  } else if (grepl("4", yscenario, fixed = TRUE) & !grepl(".4", yscenario, fixed = TRUE)) {
    y_label <- "complex_outcome"
    if (grepl(".1", yscenario, fixed = TRUE)) {
    ...
    y_label <- "simple_outcome_unobserved"
  } else {
    # not yet implemented
  }
  return(y_label)
...
# get nice m scenario label
# @param mscenario the character-valued m-scenario
get_nice_mscenario_label <- function(mscenario = "1.1") {
  missing_proportion <- ifelse(mscenario %in% c("3.1", "2.3", "2.4", "2.7", "2.8"), "80", "40")
  if (mscenario %in% c("1", "1.1", "3", "3.1")) {
    if (grepl(".1", mscenario)) {
      m_label <- "simple_mar"
    } else {
      m_label <- "simple_mar_noy"
    }
  } else if (mscenario %in% c("2.2", "2.4")) {
    m_label <- "complex_mar"
  } else if (mscenario %in% c("2.1", "2.3")) {
    m_label <- "complex_mar_noy"  
  } else if (mscenario %in% c("2.5", "2.7")){
    m_label <- "mnar_unobserved"
  } else {
    m_label <- "mnar_value"
  }
  return(paste0(m_label, "_", missing_proportion))
}
...
get_caption <- function(xscenario = "1", yscenario = "1",
  ...
  if (plasmode) {
    ...
    outcome_proportion <- ifelse(grepl(".17", yscenario, fixed = TRUE), "5\\%", 
                                 ifelse(grepl(".15", yscenario, fixed = TRUE), "1\\%", "12\\%"))
    missing_proportion <- ifelse(mscenario %in% c("3.1", "2.3", "2.4", "2.7", "2.8"), "80\\%", "40\\%")
    if (mscenario %in% c("1", "1.1", "3", "3.1")) {
      if (grepl(".1", mscenario)) {
...
get_summary_tib <- function(output, rd_mult = 1, reasonable_threshold = log(10),
  ...
  if (threshold_scale == "log") {
    threshold_txt <- paste0("ln(", exp(reasonable_threshold), ")")
  } else {
...
create_report_kable <- function(output, oracle_census_equal = TRUE, estimand = "cOR", oracle = TRUE,
  ...
  # suppress benchmark if necessary
  if (suppress_benchmark) {
    this_kable_init <- this_kable_init %>% 
      filter(!grepl("benchmark", Estimator, ignore.case = TRUE))
  }
...
get_true_values <- function(all_xym = NULL, iter = 1, sample_size = 1e6, nreps = 10,
  ...
  outcome_formulas <- get_outcome_formulas(YScenario = yscenario, plasmode = plasmode,
                                           rare_outcome = ifelse(plasmode, grepl("365day", yscenario), FALSE))
  ...
  if (isTRUE(plasmode)) {
    ...
    if (!grepl("1826day", yscenario)) {
      zs_name <- "AgeAtIndex"
    } else {
      zs_name <- "AgeAtIndex_cat"
    }
  } else {
  ...
  if (isTRUE(plasmode)) {
    num_ws <- stringr::str_count(outcome_formulas$oracle, "IndexTrtPHQ_item9_score_cat")
    beta_ws <- t(replicate(num_ws, vector("numeric", length = nreps)))
  } else {
  ...
  if (isTRUE(plasmode)) {
    if (grepl("cat", zs_name)) {
      num_pop_beta_zs <- 5
    } else {
      num_pop_beta_zs <- 1
    }
    num_pop_ws <- stringr::str_count(census_formula, "IndexTrtPHQ_item9_score_cat")
    pop_beta_zs <- t(replicate(num_pop_beta_zs, vector("numeric", length = nreps)))
    pop_beta_ws <- t(replicate(num_ws, vector("numeric", length = nreps)))
  } else {
  ...
  for (j in 1:nreps) {
    ...
    census_df <- df
    census_newdata_0 <- newdata_0
    census_newdata_1 <- newdata_1
    census_newdata_Zs_0 <- newdata_Zs_0
    census_newdata_Zs_1 <- newdata_Zs_1
    census_newdata_Ws_0 <- newdata_Ws_0
    census_newdata_Ws_1 <- newdata_Ws_1
    if (isTRUE(plasmode) & !grepl("1826day", yscenario)) {
      census_df$Charlson_cat <- factor(ifelse(census_df$Charlson == 0, 0, 1))
      census_newdata_0$Charlson_cat <- factor(ifelse(census_newdata_0$Charlson == 0, 0, 1))
      census_newdata_1$Charlson_cat <- factor(ifelse(census_newdata_1$Charlson == 0, 0, 1))
      census_newdata_Zs_0$Charlson_cat <- factor(ifelse(census_newdata_Zs_0$Charlson == 0, 0, 1))
      census_newdata_Zs_1$Charlson_cat <- factor(ifelse(census_newdata_Zs_1$Charlson == 0, 0, 1))
      census_newdata_Ws_0$Charlson_cat <- factor(ifelse(census_newdata_Ws_0$Charlson == 0, 0, 1))
      census_newdata_Ws_1$Charlson_cat <- factor(ifelse(census_newdata_Ws_1$Charlson == 0, 0, 1))
    }
    ...
    fit <- glm(as.formula(census_formula), family = "binomial", data = census_df)
    preds_0 <- predict(fit, newdata = census_newdata_0, type = "response", na.action = na.omit) 
    preds_1 <- predict(fit, newdata = census_newdata_1, type = "response", na.action = na.omit) 
    preds_0_zs <- predict(fit, newdata = census_newdata_Zs_0, type = "response", na.action = na.omit) 
    preds_1_zs <- predict(fit, newdata = census_newdata_Zs_1, type = "response", na.action = na.omit) 
    preds_0_ws <- predict(fit, newdata = census_newdata_Ws_0, type = "response", na.action = na.omit) 
    preds_1_ws <- predict(fit, newdata = census_newdata_Ws_1, type = "response", na.action = na.omit) 
    ...
    if (isTRUE(!plasmode)) {
      ...
      zs_name2 <- census_coef_names[grepl(zs_name, census_coef_names) & !grepl(":", census_coef_names, fixed = TRUE)]
      ws_name <- census_coef_names[grepl("IndexTrtPHQ_item9_score_cat", census_coef_names) & !grepl(":", census_coef_names, fixed = TRUE)]
      pop_beta_ws[, j] <- coefficients(fit)[ws_name]
      pop_beta_zs[, j] <- coefficients(fit)[zs_name2]
    }
